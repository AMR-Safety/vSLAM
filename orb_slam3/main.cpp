...
std::vector<Eigen::Vector3f> loadedMapPoints;

void visualizeMapPoints(const std::vector<Eigen::Vector3f>& points) {
...
void loadORBSLAMMap(OccupancyGrid& grid) {
    ...
    for (const auto& pos : loadedMapPoints) {
        int gx, gy;
...
int main() {
    ...
    std::thread viewerThread(visualizeMapPoints, loadedMapPoints);
