...
public:
    ...
    MapValidationResult validateMapLoading(const std::string& mapFile, OccupancyGrid& grid) {
        ...
        try {
            ...
            slamPtr = new ORB_SLAM3::System(
                ...
                "/home/keshawa/amr_safety/orb_slam3/ORB_SLAM3/Examples/Stereo-Inertial/RealSense_D435i.yaml",
    ...
    // 1. Add the MapValidationResult struct to your code
    // 2. Add validateMapLoading() as a private method to your SLAMSystem class
    // 3. Add testCoordinateTransformations() as a private method to your SLAMSystem class  
    // 4. Replace your existing loadMap() function call in main() with:
    //    if (!slamSystem.loadMapWithValidation(mapFilePath, grid)) {
    //        std::cerr << "Map validation failed. Exiting." << std::endl;
    //        return 1;
    //    }
    ...
    int main(int argc, char** argv) {
        ...
        std::cout << "Loading and validating map from: " << mapFilePath << std::endl;
        if (!slamSystem.loadMapWithValidation(mapFilePath, grid)) {
            std::cerr << "Map validation failed. Check the validation report above." << std::endl;
            return 1;
        }
        std::cout << "Map validation completed successfully!" << std::endl;
    ...
    */
    ...
    bool loadMap(const std::string& mapFile, OccupancyGrid& grid) {
        try {
            // Initialize ORB-SLAM3 in STEREO+LOCALIZATION mode (4th parameter true for localization only)
            slamPtr = new ORB_SLAM3::System(
                "/home/keshawa/amr_safety/orb_slam3/ORB_SLAM3/Vocabulary/ORBvoc.txt",
                "/home/keshawa/amr_safety/orb_slam3/ORB_SLAM3/Examples/Stereo-Inertial/RealSense_D435i.yaml",
                ORB_SLAM3::System::STEREO, true);
            ...
            std::cout << "Loading and validating map from: " << mapFilePath << std::endl;
            if (!loadMapWithValidation(mapFilePath, grid)) {
                std::cerr << "Map validation failed. Check the validation report above." << std::endl;
                return 1;
            }
            std::cout << "Map validation completed successfully!" << std::endl;
            ...
            std::cout << "Map loaded successfully. Activating localization-only mode..." << std::endl;
            slamPtr->ActivateLocalizationMode();
            std::this_thread::sleep_for(std::chrono::seconds(1));
...
void slamTrackingLoop(const SLAMSystem& slam, RobotState& robot) {
    try {
        ...
        std::cout << "Initializing stereo cameras for SLAM tracking in localization mode..." << std::endl;
        ...
        while (runningFlag) {
            try {
                ...
                // Periodically check and ensure we remain in localization mode
